...
def test_impact_filters_by_type_and_limits():
    ensure_tables()
    os.environ["IMPACT_CACHE_TTL_S"] = "120"
    tid = "tenant-imp-f"
    ...
    conn.close()
    res = impact_subgraph_for_proposal(p.proposal_id, depth=1, types=["LINKED"], max_nodes=10, max_edges=10)
    assert len(res["edges"]) >= 1
    assert all(e.get("kind") == "LINKED" for e in res["edges"])
    res2 = impact_subgraph_for_proposal(p.proposal_id, depth=1, types=["BASED_ON"], max_nodes=10, max_edges=10)
    assert all(e.get("kind") == "BASED_ON" for e in res2["edges"])
