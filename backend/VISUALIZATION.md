...
- **Типы визуализации**:
  - `geometric_shape`: Геометрические фигуры по координатам.
...
- **Персональный фидбек**: LLM анализирует историю ответов и дает комментарий (max 2 предложения) по *каждому* вопросу, объясняя ошибки или отмечая успехи.
...
- При генерации нового вопроса в LLM передаются последние 3 вопроса, чтобы исключить создание дубликатов или слишком похожих задач.
...
#### geometric_shape
Массив точек. Пример (Треугольник):
```json
{
...
#### graph
Параметры функции. Пример (Парабола):
```json
{
  "type": "graph",
  "coordinates": {
