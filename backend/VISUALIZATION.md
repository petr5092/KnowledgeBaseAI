...
- **Решение**: Перед отправкой ответа клиенту запускается алгоритм `repack_shapes` (Geometry Engine), который:
  1. Вычисляет границы (Bounding Box) каждой фигуры.
  2. Если обнаруживается наложение, фигуры автоматически "раздвигаются" по холсту с соблюдением минимальных отступов.
  3. Сохраняется исходная форма и размеры каждой фигуры, меняется только их расположение (центр).
- **Производительность**: Алгоритм работает локально за <1 мс и не создает задержек в ответе API.
- **Результат**: Пользователь всегда получает аккуратную визуализацию без визуального мусора, даже если LLM ошиблась в абсолютном позиционировании.
