...
def _apply_ops_tx(tx, tenant_id: str, ops: List[Dict[str, Any]]) -> None:
    for op in ops:
        ...
        if t in ("CREATE_NODE", "MERGE_NODE"):
            ...
            uid = str(op.get("target_id") or "")
            detach = bool(pd.get("detach"))
            if detach:
                tx.run("MATCH (n {uid:$uid, tenant_id:$tenant_id}) DETACH DELETE n", uid=uid, tenant_id=tenant_id)
            else:
                tx.run("MATCH (n {uid:$uid, tenant_id:$tenant_id}) DELETE n", uid=uid, tenant_id=tenant_id)
        elif t == "DELETE_REL":
            uid = str(op.get("target_id") or "")
            tx.run("MATCH (a {tenant_id:$tid})-[r {uid:$uid}]-(b {tenant_id:$tid}) DELETE r", uid=uid, tid=tenant_id)
...
def commit_proposal(proposal_id: str) -> Dict:
    ...
    for op in ops:
        ...
        if not tid:
            continue
        if t in ("CREATE_NODE", "MERGE_NODE", "UPDATE_NODE", "DELETE_NODE"):
            changes.append({"target_id": tid, "change_type": "NODE"})
        elif t in ("CREATE_REL", "MERGE_REL", "UPDATE_REL", "DELETE_REL"):
