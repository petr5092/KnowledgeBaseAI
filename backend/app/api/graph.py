...
async def roadmap(payload: RoadmapInput) -> Dict:
    ...
    """
    items = plan_route(payload.subject_uid, payload.progress, limit=payload.limit, tenant_id=get_tenant_id())
...
async def adaptive_questions(payload: AdaptiveQuestionsInput) -> Dict:
    ...
    """
    tid = get_tenant_id()
    roadmap = plan_route(payload.subject_uid, payload.progress, limit=payload.count * 3, tenant_id=tid)
