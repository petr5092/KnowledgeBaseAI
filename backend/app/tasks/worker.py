...
async def kb_validate_job(ctx, job_id: str, subject_uid: str | None = None, auto_publish: bool = False):
    ...
    try:
        from app.services.graph.utils import build_graph_from_neo4j
        from app.services.validation import validate_canonical_graph_snapshot
...
async def kb_rebuild_job(ctx, job_id: str, auto_publish: bool = False):
    ...
    try:
        from app.services.graph.utils import sync_from_jsonl, compute_static_weights, add_prereqs_heuristic, analyze_knowledge
    except Exception as e:
...
class WorkerSettings:
    ...
    functions = [magic_fill_job, kb_rebuild_job, kb_validate_job, vector_consume_job, outbox_publish_job]
    cron_jobs = [
