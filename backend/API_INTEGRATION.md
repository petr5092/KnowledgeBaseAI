...
Граф в Neo4j представляет собой иерархическую структуру знаний с привязанным контентом.

*   **Иерархия (Nodes)**:
    *   `Subject` (Предмет)
    *   `Section` (Раздел)
    *   `Subsection` (Подраздел)
    *   `Topic` (Тема — основной обучаемый узел)
*   **Контент (Nodes)**:
    *   `ContentUnit` (Микро-урок, тип: `concept`, `rule`, `example` и др.)
    *   `Skill` (Навык)
*   **Связи (Relationships)**:
    *   `(:Subject)-[:CONTAINS]->(:Section)-[:CONTAINS]->(:Subsection)-[:CONTAINS]->(:Topic)` — Структура предмета.
    *   `(:Topic)-[:PREREQ]->(:Topic)` — Зависимость знаний.
    *   `(:Topic)-[:HAS_UNIT]->(:ContentUnit)` — Привязка контента к теме.
    *   `(:Topic)-[:REQUIRES_SKILL]->(:Skill)` — Требуемые навыки.

*   **Примечание**: Ранее упоминалось, что уроки формируются только в рантайме. Сейчас `ContentUnit` хранятся в графе физически, что позволяет гибко переиспользовать их.
