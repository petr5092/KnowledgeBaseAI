...
*   **Задача:** При клике на узел (событие `selectNode`) открывать детальную панель.
*   **Решение:**
    *   Создан компонент `NodeDetailsSidebar`.
    *   Отображение метаданных (UID, Title, Kind) и связей (Входящие/Исходящие).
    *   Добавлена кнопка "Спросить AI об этом узле" (интеграция с чатом пока в разработке).
